...
public class CommandClearInventory extends CommandBase
{
    ...
    {
        PlayerMP entityplayermp = args.length == 0 ? getCommandSenderAsPlayer(sender) : getPlayer(sender, args[0]);
        ...
        else
        {
            ...
            else
            {
                if (j == 0)
                {
                    sender.sendMessage(new ChatComponentTranslation("commands.clear.testing", new Object[] {entityplayermp.getName(), Integer.valueOf(k)}));
                }
                else
                {
